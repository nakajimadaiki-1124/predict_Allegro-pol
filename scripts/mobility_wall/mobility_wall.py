...
This script parses a extxyz file with the dipoles per cell, 
which is generated by hysteresis.py, and determines the mobility
of domain walls through amojority rule.
"""
...
class extxyz_dipoles:
    ...
    def fold_images(self, δ=0.5):
        ...
        for frame in range(self.nframes):
            for ia in range(self.nat):
                ...
                if r[0] < -δ or r[1] < -δ or r[2] < -δ or r[0] > self.p[frame,0,0]+δ or r[1] > self.p[frame,1,1]+δ or r[2] > self.p[frame,2,2]+δ:
                ...
                self.R[frame,ia,:] += 0.5* (self.p_unit[0,:]+self.p_unit[1,:]+self.p_unit[2,:])

    def print_coordinates(self):

        # trajectory file
        str_format="{:5s} "+"{:14.10f} "*7+" \n"
        with open(self.filename[:-4]+"-centered.xyz", 'w') as f:
            for frame in range(self.nframes):
                f.write(str(self.nat)+"\n")
                f.write("\n")
                for iat in range(self.nat):
                    f.write(str_format.format(
                        self.kinds[frame][iat],
                        self.R[frame,iat,0],       self.R[frame,iat,1],       self.R[frame,iat,2],
                        self.dipoles[frame,iat,0], self.dipoles[frame,iat,1], self.dipoles[frame,iat,2],
                        self.θ[frame,iat]))
        ...
        for frame in tqdm.trange(self.nframes):
            ...
            for i in range(n_x):
                for j in range(n_y):
                    ...
                    dipoles_in_bin = []
                    for k in range(len(R)):  # Loop over all positions
                        x, y = R[k, 0], R[k, 1]
                        if x_min <= x < x_max and y_min <= y < y_max:
                            dipoles_in_bin.append(dipoles[k, :])  # Collect the dipole

                    # Raise an error if the number of dipoles does not match n_z
                    if len(dipoles_in_bin) != n_z:
                        raise ValueError(f"Expected {n_z} dipoles in bin ({i}, {j}), found {len(dipoles_in_bin)}")

                    # Calculate the average dipole in the bin
                    average_dipole = np.mean(dipoles_in_bin, axis=0)

                    # Calculate the cosine of the angle between the average dipole and the z-axis
                    z_axis = np.array([0, 0, 1])
                    cosine_angle = np.dot(average_dipole, z_axis) / (np.linalg.norm(average_dipole) + 1e-10)
                    ...
                    if save_pdf:
                        ...
                        center_x = i + 0.5
                        center_y = j + 0.5
                        ax.plot(center_x, center_y, 'o', color='black')  # Black dot at the center
    ...
    def analyze_steps_to_flip(self):
        """
        Analyze the number of steps it takes for each bin to turn from blue to red
        across the entire trajectory. Visualize the results in a 2D map.
        """

        # Initialize a dictionary to store step counts for each bin
        step_counts = {}
        n_frames = self.nframes

        # Iterate through each frame in the map_data
        for frame, frame_data in self.map_data.items():
            for (i, j), data in frame_data.items():
                cosine_angle = data['cosine_angle']

                # Initialize the bin in the step_counts dictionary if not already present
                if (i, j) not in step_counts:
                    step_counts[(i, j)] = []

                # Store the cosine angle for this bin
                step_counts[(i, j)].append(cosine_angle)

        # Calculate the number of steps to flip the sign for each bin
        steps_to_flip = np.full(len(step_counts), np.nan)  # Initialize with NaN
        for (i, j), angles in step_counts.items():
            for step in range(n_frames - 1):
                # Check if the sign has flipped between steps
                if angles[step] < 0 and angles[step + 1] >= 0:
                    steps_to_flip[list(step_counts.keys()).index((i, j))] = step + 1
                    break
                elif angles[step] >= 0 and angles[step + 1] < 0:
                    steps_to_flip[list(step_counts.keys()).index((i, j))] = step + 1
                    break

        # Plotting the results
        self.plot_flip_steps(steps_to_flip, len(step_counts))


    def plot_flip_steps(self, steps_to_flip, n_bins):
        """
        Plot a 2D map where the index corresponds to the number of steps
        required to flip the sign of the bin and save it to a PDF.
        """
        # Create a figure and axis
        fig, ax = plot_init("x", "y", "Steps to Flip")

        # Define grid dimensions
        n_x = int(np.sqrt(n_bins))  # Assume square grid for simplicity
        n_y = n_bins // n_x

        # Normalize the step counts for coloring
        norm = plt.Normalize(vmin=np.nanmin(steps_to_flip), vmax=np.nanmax(steps_to_flip))
        cmap = plt.get_cmap('viridis_r')  # Use the inverted viridis colormap

        # Loop over each bin and plot the step count inside
        for idx in range(n_bins):
            i, j = divmod(idx, n_y)
            steps_value = steps_to_flip[idx]

            # Set the color based on the number of steps
            color = cmap(norm(steps_value)) if not np.isnan(steps_value) else (1, 1, 1, 1)  # White for NaN

            # Draw the unit cell as a rectangle with the chosen color
            rect = plt.Rectangle((i, j), 1, 1, facecolor=color, edgecolor='black')
            ax.add_patch(rect)

            # Plot the number of steps inside the bin
            if not np.isnan(steps_value):
                ax.text(i + 0.5, j + 0.5, str(int(steps_value)), 
        ...
        # Add color bar for reference with integer ticks
        sm = plt.cm.ScalarMappable(cmap=cmap, norm=norm)
        sm.set_array([])
        cbar = plt.colorbar(sm, ax=ax)
        cbar.set_label('Number of Steps to Flip')
        cbar.set_ticks(np.arange(int(np.nanmin(steps_to_flip)), int(np.nanmax(steps_to_flip)) + 1))  # Integer labels

        # Save the plot to a PDF file
        with PdfPages(f"{self.system}/{self.system}-flipcount.pdf") as pdf:
            pdf.savefig(fig)
...
if system == "BaTiO3-full":
    file_extxyz = "BaTiO3-full/BaTiO3-dipoles.xyz"
    p_unit  = [[3.966084,0,0],[0,3.966084,0],[0,0,4.304025]]
