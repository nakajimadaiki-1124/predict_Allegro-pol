...
"""
Authors: A. Johansson, S. Falletta
...
if __name__ == '__main__':
    ...
    with PdfPages(system+"/"+system+".pdf") as pdf:
        ...
        plt.fill_between(Ts, mean_upfield + std_upfield, mean_upfield - std_upfield, alpha=0.5, color=colors[0])
        plt.plot(Ts, mean_downfield, label="dw→up", c=colors[1], lw=2.5)
        plt.fill_between(Ts, mean_downfield + std_downfield, mean_downfield - std_downfield, alpha=0.5, color=colors[1])
        plt.plot(Ts, (mean_downfield + mean_upfield) / 2, label="mean", c=colors[2], lw=2.5)
        plt.scatter(Ts, (mean_downfield + mean_upfield) / 2, c=colors[2], s=100, zorder=100)
